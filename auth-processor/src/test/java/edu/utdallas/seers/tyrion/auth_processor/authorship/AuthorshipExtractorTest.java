...
public class AuthorshipExtractorTest {
	...
	private static String destinationFolder = baseFolder + "cloning" + File.separator + projectName2;
	...
	@Test
	public void testGetClassAuthorContributions() throws IOException, ParseException {
		...
		String[] sourceSubFolders = new String[] { "src" + File.separator + "java" + File.separator + "main",
				...
				"src" + File.separator + "java" + File.separator + "test", "src" + File.separator + "contrib"
						+ File.separator + "zooinspector" + File.separator + "src" + File.separator + "java" };

		String[] classPaths = new String[] { destinationFolder };

		Map<String, AuthorInfo> authorInfo = new AuthorshipExtractor(destinationFolder, sourceSubFolders, classPaths)
				.getClassAuthorContributions(commits);

		String clName = "src.java.main.org.apache.zookeeper.ClientCnxn.AuthData";
		String authors[] = { "camille@apache.org", "henry@apache.org", "phunt@apache.org", "michim@apache.org",
				"mahadev@apache.org", "breed@apache.org", "fpj@apache.org", "rakeshr@apache.org" };
		...
		assertClass(authorInfo, clName, authors, contrib, total, firstCommitId, javadocAuthors, contribFirstCommit);

		clName = "src.java.main.org.apache.zookeeper.server.NettyServerCnxn";
		authors = new String[] { "camille@apache.org", "henry@apache.org", "phunt@apache.org", "michim@apache.org",
				"mahadev@apache.org", "fpj@apache.org", "rakeshr@apache.org" };
		...
		authors = new String[] { "phunt@apache.org", "michim@apache.org", "mahadev@apache.org", "fpj@apache.org",
				"shralex@apache.org" };
	...
	private void assertClass(Map<String, AuthorInfo> authorInfo, String clName, String[] authors, int[] contrib,
			int total, String firstCommitId, String[] javadocAuthors, int contribFirstCommit) {
		System.out.println("Checking class [" + clName + "]");

		Map<String, AuthorContribution> clContr = authorInfo.get(clName).getHistoryContrib();
		...
		for (int i = 0; i < authors.length; i++) {
			...
			System.out.println("checking auth: [" + auth + "] --> expected vals: [" + contrib[i] + ", " + expecPerc
					+ "], actual: [" + contr.getNumMod() + ", " + contr.getPercMod() + "]");
		...
		String commitId = authorInfo.get(clName).getFirstCommit().getCommit().getCommitId();
		assertEquals(firstCommitId, commitId);

		AuthorContribution contrib2 = authorInfo.get(clName).getFirstCommit().getContrib();
		assertEquals(contribFirstCommit, contrib2.getNumMod());
		assertEquals(1.0, contrib2.getPercMod(), 0.0);

		// ---------------------------------------------------------

		Map<String, AuthorContribution> javaDocAuthors2 = authorInfo.get(clName).getJavaDocAuthors();
		...
		for (int i = 0; i < javadocAuthors.length; i++) {
			...
			assertEquals(1.0 / javaDocAuthors2.size(), jDocContrib.getPercMod(), 0.0);
		}
